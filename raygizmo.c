			...
			case GZ_ACTION_TRANSLATE:
				{
					...
					if (GIZMO.activeAxis == GZ_ACTIVE_XYZ)
					{
						GIZMO.activeTransform->translation = Vector3Add(GIZMO.activeTransform->translation,
							Vector3Multiply(pVec, data->right));
						GIZMO.activeTransform->translation = Vector3Add(GIZMO.activeTransform->translation,
							Vector3Multiply(pVec, data->up));
					}
					else
					{
						if (GIZMO.activeAxis & GZ_ACTIVE_X)
						{
							const Vector3 prj = Vector3Multiply(pVec, data->axis[GZ_AXIS_X]);
							GIZMO.activeTransform->translation = Vector3Add(GIZMO.activeTransform->translation, prj);
						}
						if (GIZMO.activeAxis & GZ_ACTIVE_Y)
						{
							const Vector3 prj = Vector3Multiply(pVec, data->axis[GZ_AXIS_Y]);
							GIZMO.activeTransform->translation = Vector3Add(GIZMO.activeTransform->translation, prj);
						}
						if (GIZMO.activeAxis & GZ_ACTIVE_Z)
						{
							const Vector3 prj = Vector3Multiply(pVec, data->axis[GZ_AXIS_Z]);
							GIZMO.activeTransform->translation = Vector3Add(GIZMO.activeTransform->translation, prj);
						}
					}
				}
			...
			case GZ_ACTION_SCALE:
				{
					...
					else
					{
						if (GIZMO.activeAxis & GZ_ACTIVE_X)
						{
							...
							const Vector3 prj = Vector3Multiply(pVec, GIZMO.axisCfg[GZ_AXIS_X].normal);
							// data->axis[GIZMO_AXIS_X]);
							GIZMO.activeTransform->scale = Vector3Add(GIZMO.activeTransform->scale, prj);
						}
						if (GIZMO.activeAxis & GZ_ACTIVE_Y)
						{
							const Vector3 prj = Vector3Multiply(pVec, GIZMO.axisCfg[GZ_AXIS_Y].normal);
							GIZMO.activeTransform->scale = Vector3Add(GIZMO.activeTransform->scale, prj);
						}
						if (GIZMO.activeAxis & GZ_ACTIVE_Z)
						{
							const Vector3 prj = Vector3Multiply(pVec, GIZMO.axisCfg[GZ_AXIS_Z].normal);
							GIZMO.activeTransform->scale = Vector3Add(GIZMO.activeTransform->scale, prj);
						}
					}
				}
